...
//    EV<<"Start Contention "<<simTime()<<endl;

    startTime = simTime();
//    EV<<"Start Contention "<<startTime<<endl;
    ...
    EV<<"Krishan_Contention cw--"<<cw<<endl;
    EV<<"Krishan_Backoffslots --"<<backoffSlots<<endl;
...
void Contention::incrementBackoff(){
  cw = 2*(cw+1)-1;

  if (cw>1023){
      cw=1000; //minimum contion window size
  }
