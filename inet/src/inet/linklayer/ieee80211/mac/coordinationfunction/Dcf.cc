...
void Dcf::initialize(int stage)
{
    ...
    path="G:/FYP/CurrentNodeDensity/csv/verify/80211g_3.csv";
...
void Dcf::processUpperFrame(Ieee80211DataOrMgmtFrame* frame)
{
    ...
    if (pendingQueue->insert(frame)) {
        ...
        requestTime=simTime();
        ////////////////////////////////////////////////////////////////////////////////////////////////
        dcfChannelAccess->currentAccessClass = int(frame->par("currentAccessClass"));
        ///////////////////////////////////////////////////////////////////////////////////////////////
...
void Dcf::transmitFrame(Ieee80211Frame* frame, simtime_t ifs)
{
    ...
    frame->addPar("WaitInerval")=((grantedTime-requestTime)*1000000).dbl();
    frame->addPar("collision_number")= dcfChannelAccess->getCollisions();
...
//    std::string frame_ssid(s);
    ...
    data=(std::to_string(dcfChannelAccess->getBackOff()));
    data=data+","+(std::to_string(dcfChannelAccess->getIFS()));
    data=data+","+(std::to_string(dcfChannelAccess->getSlotTime()));
    data=data+","+(std::to_string(dcfChannelAccess->getCw()));
    data=data+","+(std::to_string(dcfChannelAccess->getMaxCw()));
    data=data+","+(std::to_string(dcfChannelAccess->getMinCw()));
    data=data+","+(std::to_string(dcfChannelAccess->getCollisions()));
